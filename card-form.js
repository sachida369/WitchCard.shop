...
const RAZORPAY_KEY = 'rzp_live_Hd6RirzluzFacK'; // Replace with your actual key
...
function setupSelectedCard(cardType) {
    ...
    if (cardImage) cardImage.src = config.image;
    if (cardTitle) cardTitle.textContent = config.title;
    if (cardDescription) cardDescription.textContent = config.description;
    if (cardPrice) cardPrice.textContent = '₹10';
    if (finalPrice) finalPrice.textContent = '₹10';
}
...
function initiatePayment() {
    ...
    // For demo purposes, create order on client side
    // In production, create order on your server
...
// Create order (simulate server call)
function createOrder(formData) {
    // In production, make an API call to your server to create the order
